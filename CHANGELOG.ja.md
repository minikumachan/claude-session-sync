...
## 1.17.1
**要約:** `claude -h` の「アクセス中」表示を**ライブ自動更新**に(キー操作しなくても数秒ごとにロックを読み直して反映)。別デバイスで開始/終了すると、点灯/消灯が**ほぼリアルタイム**(同期の遅延ぶんだけ)で反映されます。
- ps1: 非ブロッキング待機ループに3秒間隔のロック再読込を追加し、状態が変化したら再描画。
- sh(curses): `getch` に 1.5秒タイムアウトを設定して定期再描画。preview/権限確認/使用中警告の単発入力はタイムアウトの影響を受けないよう実キー待ちに修正。
- ファイル同期方式の性質上、デバイス間は同期(Syncthing 等)の伝播速度が下限(完全な即時ではない)。

## 1.17.0
**要約:** `claude -h` で**現在アクセス中(使用中)の会話を表示**し、**使用中の会話を開こうとすると「先に切断して」と警告して中止**するように(同時アクセスによる履歴破損の防止)。
- **アクセス中の表示。** 共有 `locks/` の有効ロック(`session=<sid>`、12h以内)を毎回読み、該当会話のメタ行に「● アクセス中: <デバイス>」を赤で表示。
- **使用中の保護。** 使用中の会話を Enter/フォーク/権限変更で開こうとすると、使用デバイスを示して「先にそのデバイス側で終了(切断)してから開き直す」よう促し中止。どうしても開く場合は **F で強行**(危険)。Windows(矢印)・mac/Linux(curses・マウス含む)両対応。

## 1.16.1
**要約:** `claude -a` の「表示・操作」が**コマンド文字列(コード)を表示するだけ**だったのを、**テキストGUIで操作してその場で実行**できるように改善。破壊的操作は予行演習や警告＋確認を挟みます。
- **同期の状態**: setup の生出力ではなく、整形したテキストパネルで表示(方式/保存先/各コンポーネントの共有状態/各トグル)。
- **共有を開始/再リンク**: projects/skills/mcp を画面上でトグル → [予行演習](ドライラン)→ 警告＋y/N 確認 →[実行]を内部で実施(setup を呼ぶ。コマンドは見せない)。
- **MCP 共有**: 状態表示/書き出し/(秘密込み書き出し・要確認)/取り込み(破壊的・要確認)をメニューから実行。
- **元の履歴先へ復元**: 対象を選び警告＋確認 → リンク解除してローカルへ復元(退避フォルダがあれば書き戻し、無ければ共有からコピー)。実体は共有側に保持。
- Win/mac 両対応(ps1=矢印キー、sh=番号メニュー)。
