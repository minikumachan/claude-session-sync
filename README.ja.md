...
- 🔒 **使用中の会話を保護**:他のデバイス/ウィンドウで開いている会話は `claude -h` で **「● アクセス中」** と表示。開こうとすると **「先に切断して」と警告して中止**します(同時アクセスによる履歴破損を防止)。
- 🏷 **会話タイトルを自動で命名**:会話の内容と言語に合わせて、分かりやすい短いタイトルに自動で改名します(後述)。
- 🔀 **デバイス切替を自動で認識＋同期検証**:別の PC で会話を再開すると、切替を会話に伝え、**このデバイス用の正しい作業パス(存在確認済)**を提示。さらに**履歴がこのデバイスに届いているか・同期競合や転送中ファイルが無いか**を確認して警告し、未完了状態での無駄なやり直しを防ぎます(後述)。
- ♻️ **再開時に前回の設定を引き継ぐ**:履歴から再開すると、その会話で前回使っていた**モデル・思考深度・権限**を自動で復元します。
- 🎚 **モデル・思考深度・権限を切替**:壁打ち/再開ごとに指定。権限は `plan`〜**完全フリー**(`--dangerously-skip-permissions`・危険・警告つき)まで。`claude -a` / `claude -h`(Tab→[r]) / `/cc-mode` から(後述)。
...
```
- **起動する会話は複数登録**できます(壁打ち＋作業中の会話、など同時に複数起動)。
- 各項目で **モデル**(既定 `sonnet`=最新)と **思考深度**(`low`/`medium`/`high`/`xhigh`/`max`、既定 `medium`)を選べます。例:壁打ちは Sonnet・思考 medium。
- **特定の会話を再開**する項目は、**その会話で使っているモデル・思考深度をそのまま**使います(上書きしません)。
...
- **その他の設定**: `claude -a` のメニューから、同期の状態確認、**会話タイトルの自動更新 ON/OFF**、**デバイス切替の通知 ON/OFF**、共有の開始/再リンク、**元の履歴先への復元**(リンク解除)も行えます(破壊的操作は安全のため手順を表示)。
- **デバイス切替の自動認識**: 別の PC で会話を再開すると、SessionStart フックが「前回は別デバイスだった」ことを検知し、**このデバイスでの対応する作業フォルダ**(例:Win `C:\Users\X\proj` ↔ Mac `/Users/Y/proj` をホーム相対で対応付け)を会話に伝えます。OS でパスが変わっても正しい絶対パスで作業が続きます。`install-hooks` 導入で有効。
