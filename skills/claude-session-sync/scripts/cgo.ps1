...
function WantCompact([string]$item){ if(($cfg['compactOnResume']) -ne 'on'){ return $false }; return (($cfg['compact'+$item]) -eq 'on') }
# 再開前に /compact を headless(`-p`)実行し、完了(プロセス終了)まで待ってから会話を開く。
function RunCompact([string]$sid){
  if(-not $sid){ return }
  $s8= if($sid.Length -ge 8){ $sid.Substring(0,8) } else { $sid }
  Write-Host ''
  Write-Host ("  コンパクトを実行中… 完了後に会話を開きます (sid {0})" -f $s8) -ForegroundColor Cyan
  try { & $real --resume $sid -p '/compact' --output-format json 2>$null | Out-Null } catch {}
...
elseif($Mode -eq 'cc'){                          # 直前の会話を再開(全デバイス横断=同期済 projects 全体で最新)
  ...
  $sid=$newest.BaseName
  if(WantCompact 'Cc'){ RunCompact $sid }   # 再開前にコンパクト(完了後に開く)
  $cargs+=@('--resume',$sid)
