<#  claude-session-sync : 自動起動 / リモート設定の対話メニュー (Windows)
    ...
    描画は ASCII のみ(罫線/❯ 等の Ambiguous 幅文字を使わない=日本語環境でも崩れない)。
...
# 表示幅(全角=2)。列揃え・切り詰めを表示幅で行う(char数だとズレる)
...
function Edit-Entry($src){
  ...
  while($true){
    ...
    Clear-Host
    Write-Host ''; Write-Host '  起動項目の編集' -ForegroundColor Cyan
    Write-Host '  ----------------------------------------' -ForegroundColor Cyan; Write-Host ''
    foreach($r in $rows){
    ...
    Write-Host ''
    Write-Host '  Up/Down 項目  Left/Right 値を変える  Enter (会話)選択  S 決定  Esc 取消' -ForegroundColor DarkGray
...
while($true){
  ...
  Write-Host ''
  Write-Host '  Up/Down 選ぶ   Enter 編集/追加/切替   Left/Right 共通設定の切替' -ForegroundColor DarkGray
  Write-Host '  D 項目削除   S 保存して有効化   Esc 中止' -ForegroundColor DarkGray
  while(-not [Console]::KeyAvailable){ Start-Sleep -Milliseconds 80; if([Console]::WindowWidth -ne $lastW){ $lastW=[Console]::WindowWidth; break } }
  if(-not [Console]::KeyAvailable){ continue }
  ...
  if("$($k.KeyChar)" -match '^[sS]$'){
    ...
    foreach($e in $entries){
      ...
      $o.remote=$e.remote; $out+=[pscustomobject]$o
    }
  ...
  if($k.Key -eq 'Enter'){
    switch($row.kind){
      ...
      'watch' { $watch = -not $watch }
    }
    continue
  }
  if($k.Key -eq 'LeftArrow' -or $k.Key -eq 'RightArrow'){
    if($row.kind -eq 'multi'){ $checkMulti = -not $checkMulti } elseif($row.kind -eq 'watch'){ $watch = -not $watch }
    continue
  }
}
