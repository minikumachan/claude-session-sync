...
description: >
  ...
  PC ログイン時の claude 自動起動(新規/特定会話の再開・多重起動チェック・リモート可)や、
  スマホ等からのトリガで Remote Control セッションを起動する常駐ウォッチャも設定できる。
...
`bootLaunch=off|new|last|<sid>`(ログイン自動起動: 何も/新規/最近の会話/特定会話)/ `bootRemote=true|false|ask`(起動時に Remote Control を付けるか)/ `bootCheckMulti=true|false`(起動前の他デバイス使用中チェック)/ `remoteWatch=true|false`(スマホ等トリガでリモート起動するウォッチャ常駐)/ `remoteWatchDir=<path>`(監視フォルダ。既定 `<share>/remote`)
...
### 5. 自動ロック + 自動タイトル(任意・フック)
`install-hooks.ps1` / `install-hooks.sh` で SessionStart/SessionEnd/**Stop** フックを `~/.claude/settings.json` に追加する。
通常の `claude` 起動で自動ロック/解除され(競合時は警告を注入)、**応答完了(Stop)ごとに会話タイトルが自動更新**される。解除 `-Uninstall`/`--uninstall`。
...
### 7. ログイン自動起動 + リモート起動(スマホから)
PC ログイン時の claude 自動起動(**複数会話を同時に**・項目ごとにモデル/思考深度/リモート)と、スマホ等からのリモート起動を**スキルで設定**する。管理者権限不要・次回ログインから有効。起動項目は `~/.claude/session-sync.boot.json`(配列・非同期=デバイス別)、共通設定は `session-sync.local.conf`(`bootCheckMulti`/`remoteWatch`/`remoteWatchDir`)に保存。
- **いちばん簡単: `claude -a`**(`install-shell-wrap` 導入時)。矢印キーで起動項目を**追加/編集/削除**できる対話メニュー(`autostart-ui.{ps1,sh}`)。↑↓ 選択・Enter 編集/追加/切替・←→ 共通設定切替・D 削除・S 保存して有効化・Esc 中止。項目ごとに **種類(新規/最近/特定)・モデル・思考深度・リモート** を設定。保存・登録は `install-autostart.* -Apply` に委譲。
- **項目の種類とモデル/思考深度**:
  - `new`(壁打ち) … 新規会話。**モデル**(`--model`, 既定 `sonnet`=最新)と**思考深度**(`--effort low|medium|high|xhigh|max`, 既定 `medium`)を指定できる。
  - `last` / `resume`(特定会話) … 会話を再開。**モデル/思考深度は付けず、その会話で使用中のものをそのまま使う**(ユーザー指定どおり)。
- **会話で設定**: ユーザーが「自動起動を設定して」等と言ったら、Claude は希望(起動する会話と数・各モデル/思考深度・リモート・多重起動チェック・スマホからの起動)を確認し、`install-autostart.*` か `claude -a` で反映する。
- **コマンド直接指定**(上級者・単一項目を素早く設定):
  - Windows: `install-autostart.ps1 -Launch new [-Model sonnet] [-Effort medium] | -Launch last | -Session <sid>` `[-Remote|-NoRemote|-RemoteMode ask] [-CheckMulti|-NoCheckMulti] [-Watch|-NoWatch]` / `-Apply` / `-Status` / `-Uninstall`
  - macOS/Linux: `install-autostart.sh --launch new [--model sonnet] [--effort medium] | --launch last | --session <sid>` `[--remote|--no-remote|--remote-mode ask] [--watch] --apply --status --uninstall`(JSON 操作に python3 使用)
- **ログイン自動起動**(`boot-launch.*`): boot.json の各項目を起動(最後の1件はこのウィンドウ、残りは各々新ウィンドウ)。起動前に**多重起動チェック**(`bootCheckMulti`): 共有 `locks/` に**他デバイス**の有効ロック(12h以内)があれば**全起動を中止**して警告(Win/Mac 同時起動=履歴破損を防止)。
- **リモート(Remote Control)**: 項目の `remote=true` で `claude --remote-control` 付き起動 → PC が起動していれば**スマホ/claude.ai から接続・操作**。`ask` は起動時に尋ねる(8秒で OFF)。要 Claude Code v2.1.51+ / claude.ai ログイン(Pro/Max)。
- **スマホからの起動 + 指定履歴で起動**(`remote-watch.*`, `-Watch`): 監視フォルダ `<share>/remote/inbox` を常駐監視し、ファイルが置かれたら `claude --remote-control`(ファイル名か中身に session-id があれば `--resume <sid>`)を起動。**追加のポート開放・外部公開は不要**(同期フォルダ経由)。**PC が起動している間のみ**有効。なお公式 **Dispatch**(Claude デスクトップアプリ+スマホ)でも「スマホ→PCで新規セッション起動」が可能で、その場合このウォッチャは不要。
- 実装: Windows は Startup フォルダの shortcut(`ClaudeSessionSync-Boot.lnk` / `-Watch.lnk`)、mac/Linux は LaunchAgent(`com.claude-session-sync.boot/watch`)/ `~/.config/autostart` の `.desktop`。

### 補助
- `detect-sync.*`: 同期フォルダ候補の検出。 `hook-lock.*` / `hook-title.*` / `title-gen.*`: フック本体(直接呼ばない)。 `boot-launch.*` / `remote-watch.*`: 自動起動/ウォッチャ本体(直接呼ばない)。 `autostart-ui.*`: `claude -a` の対話メニュー本体。
