...
`bootCheckMulti=true|false`(ログオン自動起動前の他デバイス使用中チェック)。ログオン自動起動する会話の一覧(種類/モデル/思考深度/リモート)は `~/.claude/session-sync.boot.json`(配列・非同期=デバイス別)に保存(旧 `bootLaunch`/`bootRemote` 単一キーは後方互換のみ)
...
### 補助
- `detect-sync.*`: 同期フォルダ候補の検出。 `hook-lock.*` / `hook-title.*` / `title-gen.*`: フック本体(直接呼ばない)。 `boot-launch.*`: 自動起動本体(直接呼ばない)。 `autostart-ui.*`: `claude -a` の設定メニュー本体。
