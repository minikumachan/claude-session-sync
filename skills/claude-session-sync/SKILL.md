...
- **`claude -h`**: 本スキルの履歴ブラウザ UI(`install-shell-wrap` 導入時のみ。エンジン: `history-ui.ps1` / `history-ui.sh`)。**どのシェルからでも**起動可(PowerShell・cmd.exe・Git Bash・bash/zsh)。PowerShell 以外で素の使い方ヘルプ(コードのようなテキスト)が出てしまう問題は v1.21.0 で解消。
  ...
  - **アクセス中(使用中)の表示・保護・切断**: 共有 `locks/` の有効ロック(`session=<sid>`、12h以内)を読み、該当会話のメタ行に **「● アクセス中: <デバイス>」**(赤)を表示。**操作しなくても数秒ごとに自動更新(ライブ)**=別デバイスで開始/終了すると点灯/消灯が反映(クロスデバイスは**同期(Syncthing等)の遅延ぶんだけ遅れる“ほぼリアルタイム”**。ファイル同期方式の下限)。**使用中の会話を開こうとすると警告して中止**し、「先にそのデバイス側で終了(切断)してから開き直す」よう促す(同時アクセス=`.sync-conflict` 破損の防止)。どうしても開くなら **F で強行**(危険)。
  ...
  - **🤖サブエージェントタブ**: `…/<メインセッション>/subagents/agent-*.jsonl` を**メイン会話と分離**して一覧(親IDは祖父フォルダ名から取得)。各行は `🤖 <種別>`(`attributionAgent`)＋最初の依頼文。**実行中**(transcript が `subRunWin` 秒以内に更新=既定120)は `[実行中 ← 「<親タイトル>」メインから ・ 実行元: <デバイス>（このデバイス）]`、非実行中は `[元: 「<親タイトル>」 ・ <デバイス>]`。Enter/クリック/Tab→「実行元のメイン会話を開く」で**親メイン会話**へ(サブ単体は再開対象外)、Space でプレビュー。
  - **メイン行のサブエージェント実行中表示**: メイン会話に**アクセス中デバイスが無く(ロック無し)**、かつそのサブエージェントが実行中のとき、メタ行に `[<デバイス> でサブエージェント実行中（このデバイス）]` を表示。アクセス中ロックがあればそちら優先、どちらも無ければ表示なし。公式にサブエージェント用ロックは無いため transcript の鮮度で近似(精度は同期遅延の範囲)。
- 導入/解除: `install-shell-wrap.ps1` /（Unix）`install-shell-wrap.sh`。2系統で横取りするため**全シェル・全状況**で動く: (A) プロファイル/rc に `claude` 関数を追加、(B) デバイス毎ローカルの `~/.claude/css-bin/` に shim(`claude.cmd`/`claude.ps1`/`claude`)を置き **User PATH 先頭**へ追加(cmd.exe・Git Bash・プロファイル未読込の pwsh もカバー)。**`-h`=履歴UI / `-a`=自動起動・リモート設定**を横取り、`-r` 含む他は実体 claude へ素通し(PATH から css-bin を除いて解決=再帰なし)。Windows の PATH 更新はレジストリ `REG_EXPAND_SZ` を保ち `WM_SETTINGCHANGE` を通知。解除 `-Uninstall` / `--uninstall`(関数・shim・PATH 登録をすべて除去)。新ターミナルを開くと反映。
