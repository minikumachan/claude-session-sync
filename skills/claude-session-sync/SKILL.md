...
### 3b. 全履歴を閲覧(どのパスからでも・ページ式・デバイス色分け)
`claude --resume` はカレントのプロジェクトに限定されるが、`history` は **`~/.claude/projects` を直接読む**ため、
**どのディレクトリからでも全プロジェクト・全デバイスの履歴**を扱える。各行に**由来デバイスを色＋ラベル**で表示し、
**タイトル**は「言語固定の生成タイトル(titles.map)> Claude の ai-title > 冒頭発話」の優先順で表示。
- 一覧(ページ式): `history.ps1 list [-Page N] [-PageSize N] [-Grep <語>] [-Device <名>]` / `history.sh list [--page N] [--pagesize N] [--grep ..] [--device ..]`
  - 現ページ分だけスキャンするので高速。`#` はソート済み全体の通し番号(view/resume に使える)。
- 本文表示: `history.* view <#|id>` / 再開: `history.* resume <#|id>` / パス: `history.* path <#|id>`
- **タイトル生成(言語固定)**: `history.ps1 title [-Limit N|-All|-Id <#|id>]` / `history.sh title [--limit N|--all|<id>]`
  会話内容(ai-title/冒頭)から `lang` 設定の言語で簡潔なタイトルを `claude -p` 生成し titles.map にキャッシュ。
- **デバイス識別**: cwd から `Win/<user>` `Mac/<user>` `Linux/<user>` を導出。**同機種・同ユーザーの別マシン**は
  config の `deviceName`(フックが `devices.map` に記録)で識別。`setup -DeviceName <名>` / `-Lang <ja|en|…>` で設定。

### 3c. `claude -r` を全履歴に拡張(resume-all / シェル統合)
`claude --resume` 自体は本体改変不可で、カレントのプロジェクトフォルダ限定。そこで全 .jsonl を
「同期しないローカル集約フォルダ」へハードリンクで集約し、そこで native の `--resume` を開いて全件選択可能にする。
- 直接実行: `resume-all.ps1 [-Limit N] [-Days N] [-All]`(既定は最近100件=picker高速化。`-DryRun` で集約のみ)/ `resume-all.sh [--limit N] [--days N] [--all] [--dry-run]`
- **`claude -r` をそのまま拡張**: `install-shell-wrap.ps1` /(Unix）`install-shell-wrap.sh` でプロファイル/rc に
  `claude` 関数を追加 → 以後 `claude -r`(`--resume`)が全履歴ピッカーになる。その他の引数は実体の claude へ素通し。解除は `-Uninstall` / `--uninstall`。
- 集約フォルダは `.stignore` / `.gitignore` で同期除外(他デバイスを汚さない)。デバイス数に上限はない(Syncthing は多数ピア、git は多数クローンに対応)。
