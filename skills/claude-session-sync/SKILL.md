...
### 7. ログイン自動起動 + リモート起動(スマホから)— `install-autostart.*`
...
### 補助
- `detect-sync.*`: 同期フォルダ候補の検出。 `hook-lock.*` / `hook-title.*` / `title-gen.*`: フック本体(直接呼ばない)。 `boot-launch.*` / `remote-watch.*`: 自動起動/ウォッチャ本体(直接呼ばない)。
