...
- **適切な作業パスの推定**: 前回 cwd がローカルに在ればそれ、無ければ**ホーム相対の同一構造**(例 `C:\Users\X\proj` ↔ `/Users/Y/proj`)をこのデバイスのホーム配下で探して提示。見つからなければ現在地を使うよう促す。
- これにより、OS でパス表記が変わっても Claude が**このデバイスの正しい絶対パス**で作業でき、デバイスが切り替わった事実も会話に伝わる。
