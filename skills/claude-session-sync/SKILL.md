...
- **`claude -h`**: 本スキルの履歴ブラウザ UI(`install-shell-wrap` 導入時のみ。エンジン: `history-ui.ps1` / `history-ui.sh`)。**どのシェルからでも**起動可(PowerShell・cmd.exe・Git Bash・bash/zsh)。PowerShell 以外で素の使い方ヘルプ(コードのようなテキスト)が出てしまう問題は v1.21.0 で解消。
  ...
  - **再開先フォルダ(v1.31.0)**: Enter/行クリックの再開は既定で **その履歴が最後にいた作業フォルダ**(transcript 末尾の `cwd` を取得。存在しなければ `ch` 実行フォルダにフォールバック)へ移動してから `claude --resume`。**`ch` を実行した場所で開きたいときは Tab メニューの `[c] このフォルダ(chを実行した場所)で再開`**。フォーク/権限変更再開/文脈引き継ぎ新規/サブの実行元メインを開く も同じ既定(最後のフォルダ)。この対象フォルダへ取り込み(Import)してから、コンパクト(有効時)→ 再開の順で開く。
