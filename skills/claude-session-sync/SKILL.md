...
`titleApplyNative=on|off`(既定 on。再開系 `cc`/`ch` で `titles.map` のタイトルを Claude Code のネイティブ表示名にも適用 → `--name`=プロンプト枠/`/resume`、リモート ON 時は `--remote-control "<タイトル>"`=スマホ/claude.ai のリモート名。新規 `c`/`cfp` は対象外=起動時に内容が無いため)
`autoRead=on|off`(起動時フォルダ自動読み込み master・既定 off)/ `autoReadMode=confirm|auto`(毎回確認 or 自動送信)/ `autoReadPath=<フォルダ>` / `autoReadKind=<種別ラベル 例 Obsidian Vault『…』>` / 方式別 `autoReadC`・`autoReadCfp`(cp 共有)・`autoReadCc`・`autoReadCh`(=on|off・既定 off)。有効時、起動する claude の**初回プロンプト**に「<パス>(<種別>)の構成と主要ノートを読み全体像を把握→要点を報告」を注入(confirm は起動前に Enter=送信/↓+Enter=送信しない を選択)
`compactOnResume=on|off`(再開時コンパクト master・既定 off)/ 方式別 `compactCc`・`compactCh`(=on|off・既定 off)。有効時、再開系(`cc`/`ch`)で会話を開く**前に** `claude --resume <sid> -p "/compact"` を headless 実行し、**完了(プロセス終了)を待ってから**対話セッションを開く(履歴を要約圧縮した状態で開始)。`ch` の分岐(fork)は元会話を変えないため除外。コンパクトはトークンを消費し起動が数十秒遅くなる
...
- **`claude -h`**: 本スキルの履歴ブラウザ UI(`install-shell-wrap` 導入時のみ。エンジン: `history-ui.ps1` / `history-ui.sh`)。**どのシェルからでも**起動可(PowerShell・cmd.exe・Git Bash・bash/zsh)。PowerShell 以外で素の使い方ヘルプ(コードのようなテキスト)が出てしまう問題は v1.21.0 で解消。
  ...
  - 操作: 文字入力=検索 / Backspace=消去 / Esc=クリア(空なら終了)/ ↑↓ 選択 / ←→ タブ / PageUp,PageDown ページ / Enter 再開 / Space 内容プレビュー / **Tab=操作メニュー**。
    mac/Linux(curses)は**マウスのホイール/クリック**にも対応。Windows はキーボード操作。
  - **再開先フォルダ(v1.31.0/クロスデバイス強化 v1.32.0)**: Enter/行クリックの再開は既定で **その履歴がこの端末で最後にいた作業フォルダ**へ移動してから `claude --resume`。**別デバイスで進めた続き**でも、その会話は共有などで**この端末にも存在するはず**という前提で対応フォルダを多段解決して自動で `cd` する:
    1. **`sessionpaths.map`**(`<share>/sessions/`。`sid<TAB>device<TAB>cwd<TAB>時刻`)にこの端末の記録があれば最優先。
    2. transcript に刻まれた `cwd` 群(末尾2000行・重複除去・新しい順)のうち**この端末に実在**するもの。
    3. 別デバイスの絶対パスを**この端末へ変換**して実在するもの＝**ホーム相対**(`~/rel`)＋**共有フォルダ相対**(共有の葉名 例 `000_Share` 以降を この端末の共有ルートへ)。
    4. 見つからなければ **`ch` 実行フォルダ**。
    見つけた対象フォルダへ取り込み(Import)→(有効時)コンパクト→再開の順で開く。フォーク/権限変更再開/文脈引き継ぎ新規/サブの実行元メインを開く も同じ既定。**`ch` を実行した場所で開きたいときは Tab メニューの `[c] このフォルダ(chを実行した場所)で再開`**。
    - **記録(「履歴に刻む」)**: `sessionpaths.map` は **`hook-devswitch`(全起動方式の SessionStart)** と **`ch` 起動時**の両方が会話×デバイスの作業フォルダを upsert。一度その端末で開けば次回はステップ1で即当たる。transcript(`.jsonl`)自体は Claude Code 所有の形式なので直接書かず、同等効果のサイドカー map 方式(transcript は元々各行に `cwd` を持つのでステップ2の材料にもなる)。
  - **Tab の操作メニュー**(選択項目に対して):
    - **`[c]` このフォルダ(chを実行した場所)で再開**: 既定(前回いたフォルダ)ではなく、`ch` を起動したフォルダへ取り込んで `claude --resume`(=v1.30.x までの挙動)。
...
- **起動時フォルダ自動読み込み**: `claude` 起動時に指定フォルダ(Obsidian Vault 等)の構成と主要ノートを読み全体像を把握するよう **初回プロンプト**を注入。master(`autoRead`)/ 送信のしかた(`autoReadMode`=毎回確認/自動送信)/ 読み込む場所(`autoReadPath`・**ネイティブフォルダ選択**)/ 種別ラベル(`autoReadKind`)/ 起動方式ごと(`autoReadC`・`autoReadCfp`(cp 共有)・`autoReadCc`・`autoReadCh`、既定 off)。毎回確認は起動前に内容を表示し **Enter=送信 / ↓+Enter=送信しない**。`/css autoread on|off | mode auto|confirm | <c|cfp|cc|ch> on|off`(パス/種別は `/css gui`)。
- **日本語タイトルをネイティブ/リモート名に適用(再開時)**(`titleApplyNative`、既定 ON): `cc`/`ch` 再開時に `titles.map` のタイトルを `--name`(プロンプト枠/`/resume`)と `--remote-control "<名前>"`(スマホ/claude.ai)へ渡す。英語の自動命名(例「Implement session」)を防ぐ。新規 `c`/`cfp` は起動時に内容が無いため対象外(数ターン後 title-gen が日本語化→次回再開から反映)。`/css titlenative on|off`。
- **再開時コンパクト**(`compactOnResume` master + `compactCc`/`compactCh`、既定 OFF): `cc`/`ch` で会話を**開く前に** `claude --resume <sid> -p "/compact"` を headless 実行し、**完了を待ってから**対話セッションを開く(履歴を要約圧縮して開始)。`ch` の分岐(fork)は元会話を変えないため除外。トークン消費＋起動が数十秒遅くなる点に注意。`/css compact on|off | <cc|ch> on|off`。起動ショートカット設定の「再開時コンパクト」で master/cc/ch をトグル。
...
`claude --resume`(=`claude -r`)は他OS/他フォルダの会話を自動表示しない → **`claude -h` で全履歴から選び Enter**で続きから再開する。既定では **その履歴が最後にいた作業フォルダ**へ取り込んで `claude --resume`(v1.31.0。そのパスがこの端末に無ければ `ch` 実行フォルダにフォールバック)。`ch` を実行した場所で開きたいときは Tab メニューの `[c]`。

## セキュリティ(信頼境界)
本ツールは履歴・マップ・MCP定義を**共有フォルダ / git remote**で同期する。したがって**同期される側(別デバイス・同期アカウント・git remote・共有相手)のいずれかが汚染された場合**が攻撃面で、単独ユーザーでも「1台侵入→同期経由で全台へ横展開」を想定する。攻撃者が書き換えうるデータ＝transcript(`cwd`/`aiTitle`/本文)、`<share>/sessions/*.map`(titles/carryover/sessionpaths/lastseen/launchopts/devices/favorites)、`<share>/locks/*.lock`、`<share>/mcp/servers.json`。ローカル conf・`boot.json` は非同期=ユーザ設定として扱う。v1.33.0 でこの前提の下、次を強化(詳細は CHANGELOG 1.33.0):
- **昇格権限(`--dangerously-skip-permissions`/`bypassPermissions`)は共有 `launchopts.map` から継承しない**。対話ピッカーかローカル `boot.json` の明示指定のみ昇格可。
- **フック(`hook-lock`/`hook-devswitch`)は共有ファイル由来の値をサニタイズ**(制御文字/ESC除去・長さ制限・固定テンプレ)してから Claude 文脈へ→プロンプトインジェクション遮断。
- **ログイン自動起動(`boot-launch`)は引数配列/クオート**で組み立て→値経由のコマンド注入を防止。
- **`title-gen` は plan モード＋抜粋を未信頼フェンス**、sid は UUID 検証。**`mcp-sync` は env/headers/args を秘密検出**し import は command/args 差分を表示。
- **cmd.exe シムは PATH 明示解決(cwd 非探索)**=バイナリ設置RCE防止(再導入で反映)。
- **表示前に端末エスケープ除去**(タイトル/デバイス/プレビュー)。**クロスデバイス再開はローカル記録優先・未信頼先は起動前に確認**。
