...
PC ログイン時の claude 自動起動と、スマホ等からのリモート起動を**スキルで設定**する。管理者権限不要・次回ログインから有効。設定は `session-sync.local.conf`(同期しない=デバイス別)に保存。
- **いちばん簡単: `claude -a`**(`install-shell-wrap` 導入時)。矢印キーだけで設定できる対話メニュー(`autostart-ui.{ps1,sh}`)が開く。↑↓ で項目・←→ で値・Enter で保存して有効化・Esc で中止・Tab で全解除。「特定の会話を毎回」は直近の会話一覧から選べる。値の保存・登録は内部で `install-autostart.*` に委譲。
- **会話で設定**: ユーザーが「自動起動を設定して」等と言ったら、Claude は AskUserQuestion で4点(①どの会話で起動: なし/新規/最近/特定 ②リモート: オン/オフ/起動時に尋ねる ③多重起動チェック ④スマホからの起動)を確認し、`install-autostart.*` を対応オプションで実行する。
- **コマンド直接指定**(上級者・スクリプト用):
  - Windows: `install-autostart.ps1 -Launch new|last [-Session <sid>] [-Remote|-NoRemote|-RemoteMode ask] [-CheckMulti|-NoCheckMulti] [-Watch|-NoWatch] [-WatchDir <path>]` / 状態 `-Status` / 解除 `-Uninstall`
  - macOS/Linux: `install-autostart.sh --launch new|last [--session <sid>] [--remote|--no-remote|--remote-mode ask] [--watch] …` / `--status` / `--uninstall`
- **ログイン自動起動**(`boot-launch.*`): `bootLaunch` に従い起動。`new`=新規会話 / `last`=最近の会話を再開 / `<sid>`=特定会話を毎回再開。起動前に**多重起動チェック**(`bootCheckMulti`): 共有 `locks/` に**他デバイス**の有効ロック(12h以内)があれば**起動を中止**して警告(Win/Mac 同時起動=履歴破損を防止)。
- **リモート(Remote Control)**: `bootRemote=true` で `claude --remote-control` 付き起動 → PC が起動していれば**スマホ/claude.ai から常時接続・操作**できる。`ask` は起動時に有効化するか尋ねる(8秒で OFF)。要 Claude Code v2.1.51+ / claude.ai ログイン。
- **スマホからの起動 + 指定履歴で起動**(`remote-watch.*`, `-Watch`): 監視フォルダ `<share>/remote/inbox` を常駐監視し、ファイルが置かれたら `claude --remote-control`(ファイル名か中身に session-id があれば `--resume <sid>` も付与)を起動。起動後は claude アプリ/claude.ai に当該リモートセッションが現れる。
  - 置き方: スマホで同期フォルダの `inbox` に**1ファイル置くだけ**(新規=任意名 例 `wake.trig` / 特定会話=ファイル名か中身に UUID)。**追加のポート開放・外部公開は不要**(同期フォルダ経由)。VPN(Tailscale)/Telegram 等の別経路にしたい場合はこのウォッチャを差し替える。
  - 仕組み上 **PC が起動している間のみ**有効。完全シャットダウンからの遠隔起動は別途 WoL 等が必要。
