...
**どのディレクトリからでも全プロジェクト・全デバイスの履歴**を扱える。
- 一覧: `history.ps1 list [-Grep <語>] [-Limit N]` / `history.sh list [--grep ..] [--limit N]`
- 本文表示: `history.ps1 view <#|id>` / `history.sh view <#|id>`
- 再開: `history.ps1 resume <#|id>`(現フォルダへ取り込み → `claude --resume <id>`)
- パス表示: `history.ps1 path <#|id>`
...
- 直接実行: `resume-all.ps1`(`-DryRun` で集約のみ)/ `resume-all.sh [--dry-run]`
