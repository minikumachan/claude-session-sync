...
- **`claude -h`**: 本スキルの履歴ブラウザ UI(`install-shell-wrap` 導入時のみ。エンジン: `history-ui.ps1` / `history-ui.sh`)。**どのシェルからでも**起動可(PowerShell・cmd.exe・Git Bash・bash/zsh)。PowerShell 以外で素の使い方ヘルプ(コードのようなテキスト)が出てしまう問題は v1.21.0 で解消。
  ...
  - **行の表記(簡潔)**: 各行は2行。タイトル行=`🤖`(サブのみ)+`★`(お気に入り)+タイトル。メタ行=メイン:`<デバイス> · N msg · 時刻 · プロジェクト`/サブ:`🤖<種別> · <デバイス> · 時刻 · <状態>`。状態は記号+色で簡潔に: **🔒**=使用中 / **🤖▶**=サブ実行中 / **▶**=このサブ実行中 / **←**=元会話 / **(自)**=この端末。最下部に凡例 `🤖サブ ▶実行中 🔒使用中 ←元会話`。長い日本語マーカーは廃止(v1.22.2)。**メイン/サブはどのタブでも 🤖 の有無で判別**。
  - **アクセス中(使用中)の表示・保護・切断**: 共有 `locks/` の有効ロック(`session=<sid>`、12h以内)を読み、該当会話のメタ行に **「🔒<デバイス>」**(赤、自端末は `(自)`)を表示。**操作しなくても数秒ごとに自動更新(ライブ)**=別デバイスで開始/終了すると点灯/消灯が反映(クロスデバイスは**同期(Syncthing等)の遅延ぶんだけ遅れる“ほぼリアルタイム”**。ファイル同期方式の下限)。**使用中の会話を開こうとすると警告して中止**し、「先にそのデバイス側で終了(切断)してから開き直す」よう促す(同時アクセス=`.sync-conflict` 破損の防止)。どうしても開くなら **F で強行**(危険)。
...
- **起動ショートカット**(install-shell-wrap が全シェルに導入。関数/doskey で PATH より優先): **`c`**=現在地で claude 起動 / **`cfp`**=固定パス(`launchPath`)で起動 / **`ch`**=履歴UI / **`ca`**=設定。`c`・`cfp` は `remoteC`/`remoteCfp` が ON(既定)なら `--remote-control` を付与(=スマホ操作可能な状態)。共通ランチャ `cgo.ps1`/`cgo.sh` が conf を読み実体 claude(css-bin 除外)を起動。`cp` は copy と衝突するため不採用、`-p` は無関係(`cfp` は print でなく対話起動)。固定パス・リモートは `claude -a` → 起動ショートカット設定。
...
- **起動ショートカット設定**: **固定パス(cfp)** の場所を**ネイティブのフォルダ選択ダイアログ**(Win=エクスプローラー BrowseForFolder / mac=choose folder / Linux=zenity・kdialog)で設定。**c / cfp のリモートコントロール ON/OFF**(起動方式ごと・既定ON)。conf: `launchPath` / `remoteC` / `remoteCfp`。
